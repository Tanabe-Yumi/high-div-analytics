...
function mapToMetrics(history: FinancialHistory): EvaluationMetrics {
    ...
    const operatingProfitMargin = history.sales ? (history.operating_profit / history.sales) * 100 : 0;
...
export async function getStocks(): Promise<Stock[]> {
    ...
    const stocks: Stock[] = stocksData.map((s) => {
        ...
        let metrics: EvaluationMetrics = {
            sales: 0, operatingProfitMargin: 0, eps: 0, equityRatio: 0,
            operatingCashFlow: 0, cash: 0, dividendPerShare: 0, payoutRatio: 0
        };
...
export async function getFinancialHistory(code: string): Promise<FinancialHistory[]> {
